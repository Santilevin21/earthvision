...
def calcularHumedad(latitud, longitud, fechaInicio, fechaFin):
    ...
    humedad = image.normalizedDifference(["B8", "B11"]) #Calculamos la humedad del suelo (NDMI), (Esto todavia es una imagen)

    humedadEnNumeros = humedad.reduceRegion(reducer = ee.Reducer.mean(), geometry = boundedValue, scale = 10, maxPixels = 1e9).get("nd").getInfo() #Transformamos la imagen en un numero, que es el promedio de la humedad del suelo en toda la zona agricola

    return humedadEnNumeros #Devolvemos el numero que representa la humedad del suelo

@app.route("/")
def backend():
    latitud = -32.40
    longitud = -62.10
    return str(calcularHumedad(latitud, longitud, "2024-01-01", "2024-02-03"))
app.run()